<template>
  <div class="mx-20">
    ...
    <div v-else class="w-64 m-3 bg-slate-400 rounded-md overflow-hidden h-20 hover:shadow-md flex space-x-5 p-3 mt-3"
      ...
      <div class=" justify-end ml-10 content-end">
        <button @click="deletecontact(contact.id)" class="relative content-end  justify-end items-end text-3xl text-left">:</button>
      </div>
    </div>
    </div>
  </div>
